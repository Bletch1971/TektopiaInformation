...
public class CommandMerchantSpawn extends CommandMerchantBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		if (world == null || world.isRaining() || Village.isNightTime(world)) {
			notifyCommandListener(sender, this, MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".badconditions");
			LoggerUtils.info(TextUtils.translate(MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".badconditions"), true);
			return;
		}
		...
		if (spawnPosition == null) {
			notifyCommandListener(sender, this, MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".noposition");
			LoggerUtils.info(TextUtils.translate(MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".noposition"), true);
			return;
		}
        ...
        if (entityList.size() > 0) {
			notifyCommandListener(sender, this, MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".exists");
			LoggerUtils.info(TextUtils.translate(MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".exists"), true);
			return;
        }
        
		// attempt to spawn the merchant
		if (!TektopiaUtils.trySpawnEntity(world, spawnPosition, (World w) -> new EntityMerchant(w))) {
			notifyCommandListener(sender, this, MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".failed");
			LoggerUtils.info(TextUtils.translate(MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".failed"), true);
			return;
		}
		
		notifyCommandListener(sender, this, MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".success", TektopiaUtils.formatBlockPos(spawnPosition));
		LoggerUtils.info(TextUtils.translate(MerchantCommands.COMMAND_PREFIX + COMMAND_NAME + ".success", TektopiaUtils.formatBlockPos(spawnPosition)), true);
	}
